...
* **Hyperparameter learning.** Solve a regularised problem inside
  a training loop and let gradient descent on the upstream loss
  learn the regulariser.
...
3. The Jacobian of the optimum with respect to the data is then a
   ...
   factorisation or a matrix-free LSQR iteration.
...
a zero cone of size 1. Since $t$ is being minimised against a
...
LP — a profit-maximising production plan with a labour and a
materials budget — and ask, *given the optimum, by how much would
the profit change per extra unit of labour?* That number is the
dual variable on the labour constraint. `diffcp` lets you compute
...
`d$dx` says that an extra unit of labour increases $x_1$ by 0.6
and decreases $x_2$ by 0.2, and the corresponding profit change
is $0.8$ per unit of labour: the labour-budget shadow price,
recovered from `D` rather than from the dual variable $y$.

The advantage of going through `D` instead of the dual: it
generalises. We can ask the same question about non-trivial
...
`mode = "dense"` factors `M` once on the first call and amortises
...
* **`mode = "dense"`**: build $M$ as a dense matrix, factor with
  Eigen's LDLT, and reuse the factorisation across `D` / `DT`
...
* **PSD vectorisation.** `vec_symm()` packs the lower triangle in
...
* **Quadratic objectives.** `solve_only(P = P)` accepts a
  ...
  support `P` directly; CVXR's reduction chain canonicalises QPs
...
parameter canonicalisation) and reports gradients keyed by
...
If you use `diffcp` in academic work, please cite the original
paper:
