...
- **No `lpgd` derivative modes**, no batch APIs, no `lsmr` mode, and no
  ECOS solver branch. The supported modes are `dense` and `lsqr`.
- **Cone projections in R, Jacobians in C++.**
  [`pi()`](https://bnaras.github.io/diffcp/reference/pi.md) and the
  per-cone projections live in R (matching Python’s split between
  `cones.py` for projections and `cones.cpp` for Jacobians). All
  Jacobian, M, and LSQR machinery is in C++ via RcppEigen.
